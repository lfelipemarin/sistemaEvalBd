...
//
//	}
    public void registrarProfesorGrupo(ConexionDb conn, Usuario usuario, Grupo grupo) {
        try {

            int codigoProfesor = usuario.getCodigo();
            int codigoGrupo = grupo.getCodigo();

            System.out.println("valores obtenidos a actualizar: " + codigoProfesor + " " + codigoGrupo);
            PreparedStatement pstm = conn.getConnection().prepareStatement("update grupo "
                    + "set profesor = ? "
                    + "where codigo = ? ");
            pstm.setInt(1, codigoProfesor);
            pstm.setInt(2, codigoGrupo);
